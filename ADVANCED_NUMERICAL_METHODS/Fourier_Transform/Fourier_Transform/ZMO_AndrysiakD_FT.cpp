...
int main(int argc, char* argv[])
{
	...
	for (size_t i = 0; i < N; ++i)
	{
		...
		for (size_t j = 0; j < N; ++j)
		{
			...
			temp_Re = data[j].Get_Re() * cos((2 * M_PI * i * j) / N);
			temp_Im = data[j].Get_Re() * sin((2 * M_PI * i * j) / N);
			inverse_Re = /*(1 / (2 * M_PI)) **/ temp_Re * cos((M_PI * j));
			inverse_Im = /*(1 / (2 * M_PI)) **/ temp_Im * sin((M_PI * j));
		}
		//Cn[i] = (temp_Re + temp_Im);
		if (inverse_Re < 0.01f && inverse_Re > -0.01f)
		{
			inverse_Re = 0.0f;
		}
		if (inverse_Im < 0.01f && inverse_Im > -0.01f)
		{
			inverse_Im = 0.0f;
		}
		_STD cout << "i: " << i << " Real: " << inverse_Re << " Imaginary: " << inverse_Im << NEW_LINE;
	}
