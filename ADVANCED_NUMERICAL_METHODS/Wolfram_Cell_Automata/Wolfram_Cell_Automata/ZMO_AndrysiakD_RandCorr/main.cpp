...
int main(int argc, char* argv[])
{
	...
	for (size_t i = 0; i < SIZE; ++i)
	{
		array_1[i] = (float)rand() / RAND_MAX;
	}

		
	for (size_t i = 0; i < SIZE; ++i)
	{
		array_2[i] = (float)rand() / RAND_MAX;
	}

	std::fstream file_out;
	file_out.open("2_arrays.csv", std::ios_base::out);
	const size_t correlation_between{3};
	size_t counter{};
	float temp_array[correlation_between]{};
	for (size_t i = 0; i < SIZE; ++i)
	{
		if (correlation_between > 1)
		{
			if (i % correlation_between == 0)
			{
				file_out << temp_array[0] << ',' << array_2[i] << NEW_LINE;
				counter = 0;
			}
			else
			{
				temp_array[counter] = array_1[i];
				++counter;
			}
		}
		else
		{
			if (i < (SIZE - 1))
			{
				file_out << array_1[i] << ',' << array_2[i + 1] << NEW_LINE;
			}
		}
	}
	file_out.close();
