...
{
	for (typename std::vector<std::vector<Literal>>::iterator vec_iterator = this->Data.begin(); vec_iterator != this->Data.end(); ++vec_iterator)
	{
		if ((*vec_iterator).size() == 2)
		{
			Add_To_Knowledge((*vec_iterator)[0].Get_Value());
			//(*vec_iterator)[0].Set_Value();
			//(*vec_iterator)[1].Set_Status();
		}
	}
}
