...
void SAT::DPLL::Find_Unaries()
{
	for (typename std::vector<std::vector<int64_t>>::iterator vec_iterator = this->Data.begin(); vec_iterator != this->Data.end(); ++vec_iterator)
	{
		if ((*vec_iterator).size() == 2)
		{
			...
			for (typename std::vector<std::vector<int64_t>>::iterator vec_iterator_second = this->Data.begin(); vec_iterator_second != this->Data.end(); ++vec_iterator_second)
			{
				std::vector<int64_t>::iterator it{};
				it = std::find(vec_iterator_second->begin(), vec_iterator_second->end(), (*vec_iterator)[0]);
				if (it != vec_iterator_second->end())
				{
